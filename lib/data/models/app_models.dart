...
class ProjectSubmissionModel {
  ...
  factory ProjectSubmissionModel.fromJson(Map<String, dynamic> json) {
    ...
    return ProjectSubmissionModel(
      ...
      teamName: (json['teams'] is Map ? json['teams']['team_name'] : null)?.toString() ?? json['team_name']?.toString() ?? '',
      leaderId: (json['submitted_by'] ?? json['leader_id'])?.toString() ?? '',
      leaderName: (json['profiles'] is Map ? json['profiles']['full_name'] : null)?.toString() ?? json['leader_name']?.toString() ?? '',
      ...
      imageUrl: (json['project_images'] is List && (json['project_images'] as List).isNotEmpty ? (json['project_images'][0] as Map)['public_url'] : null)?.toString() ?? (json['image_url'] ?? json['public_url'])?.toString(),
      status: statusStr,
